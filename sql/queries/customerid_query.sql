...
FROM (
    ...
    INNER JOIN (
        ...
        QUALIFY ROW_NUMBER() OVER (ORDER BY CUSTOMER_ID) BETWEEN {start} AND {end}
    ) CU ON CU.CUSTOMER_ID = TRIM(TO_CHAR(A.ID_CLIENTE))
    WHERE B.ID_DIA BETWEEN CAST('20240601' AS DATE FORMAT 'yyyymmdd') AND CAST('20240812' AS DATE FORMAT 'yyyymmdd')
    GROUP BY
        1, 2, 3, 4, 5, 6, 7
) AS A
GROUP BY
